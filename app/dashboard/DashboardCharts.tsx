...
const METRICS = [
  { key: "leads",          label: "Liidit",              color: "#242835" },
  { key: "first_meetings", label: "Ykköstapaamiset",     color: "#929f8e" },
  { key: "all_meetings",   label: "Kaikki tapaamiset",   color: "#8ebf21" },
  { key: "deals",          label: "Kaupat",              color: "#0e3716" },
] as const;
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {METRICS.map((m) => {
          ...
          return (
            <div key={m.key} className="bg-white border border-mist p-5">
              <p className="text-xs text-sage uppercase tracking-wider mb-1">{m.label} (viime kk)</p>
              <p className="text-3xl font-bold text-navy">{val}</p>
              {diff !== null && (
                <p className={`text-xs mt-1 font-medium ${diff >= 0 ? "text-forest" : "text-[#9c3a3a]"}`}>
      ...
      {/* Kauppaennuste */}
      <div className="bg-white border border-mist p-6 mb-6">
        <div className="flex items-start justify-between mb-1">
          <h2 className="text-sm font-semibold text-navy">Kauppaennuste — perustuu ykköstapaamisiin (1 kk viive)</h2>
          <span className="text-xs text-sage">Ennuste käyttää viim. 3 kk tapaamiskeskiarvoa: {recentMeetingsAvg} kpl/kk</span>
        </div>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={forecastChartData} margin={{ top: 4, right: 16, bottom: 0, left: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#d9ddd8" />
            <XAxis dataKey="label" tick={{ fontSize: 11 }} interval="preserveStartEnd" />
            <YAxis tick={{ fontSize: 11 }} allowDecimals={false} width={30} />
            <Tooltip />
            <Legend wrapperStyle={{ fontSize: 12 }} />
            <ReferenceLine x={forecastStartLabel} stroke="#d9ddd8" strokeDasharray="4 4" />
            <Line type="monotone" dataKey="actual" stroke="#0e3716" strokeWidth={2} dot={{ r: 3 }} name="Toteutuneet kaupat" connectNulls={false} />
            <Line type="monotone" dataKey="fitted" stroke="#929f8e" strokeWidth={2} strokeDasharray="4 4" dot={false} name="Mallin sovite" connectNulls={false} />
            <Line type="monotone" dataKey="forecast" stroke="#8ebf21" strokeWidth={2} strokeDasharray="5 5" dot={{ r: 4 }} name="Ennuste (3 kk)" connectNulls />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="flex flex-col gap-6">
        {METRICS.map((m) => (
          <div key={m.key} className="bg-white border border-mist p-6">
            <h2 className="text-sm font-semibold text-navy mb-4">{m.label}</h2>
